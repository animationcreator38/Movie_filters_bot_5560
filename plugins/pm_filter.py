...
async def next_page(bot, query):
    ...
    if settings['button']:
        ...
        btn.insert(0, [
            InlineKeyboardButton("🤷‍♂️ 𝙃𝙤𝙬 𝙏𝙤 𝘿𝙤𝙬𝙣𝙡𝙤𝙖𝙙 𝙈𝙤𝙫𝙞𝙚 🤷", url=await get_tutorial(query.message.chat.id))
        ])
    else:
        btn = []
        btn.insert(0, 
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("आपने अभी तक 😠 Updates Channel join नहीं किया है। \nYou have not joined 😠Updates channel yet....", show_alert=True)
            return
    ...
    # don't change anything without contacting me @LazyDeveloperr
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/cinewood_1")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def auto_filter(client, msg, spoll=False):
    ...
    if settings["button"]:
        ...
        btn.insert(0, [
            InlineKeyboardButton("🤷‍♂️ 𝙃𝙤𝙬 𝙏𝙤 𝘿𝙤𝙬𝙣𝙡𝙤𝙖𝙙 𝙈𝙤𝙫𝙞𝙚 🤷", url=await get_tutorial(message.chat.id))
        ])
    else:
        btn = []
    if offset != "":
    ...
    if imdb:
        ...
        if not settings["button"]:
            cap+="<b>\n\n<u>⬇️ Your Movie Files ⬇️</u></b>\n"
            for file in files:
                cap += f"<b>\n📁 <a href='https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}'>[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n</a></b>"
    else:
        if settings["button"]:
            cap = f"<b>🔍 𝐒𝐞𝐚𝐫𝐜𝐡𝐢𝐧𝐠 𝐁𝐲 : {search}\n\n💬 𝐒𝐞𝐚𝐫𝐜𝐡𝐞𝐝 𝐁𝐲 : {message.from_user.mention}\n\n⏳ 𝐒𝐞𝐚𝐫𝐜𝐡𝐞𝐝 𝐓𝐢𝐦𝐞 : {remaining_seconds} sᴇᴄᴏɴᴅs\n\nᴘᴏᴡᴇʀᴇᴅ ʙʏ ☞ : {message.chat.title} \n\n⚠️ ᴀꜰᴛᴇʀ 5 ᴍɪɴᴜᴛᴇꜱ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ᴅᴇʟᴇᴛᴇᴅ 🗑️\n\n</b>"
        else:
            cap = f"<b>🔍 𝐒𝐞𝐚𝐫𝐜𝐡𝐢𝐧𝐠 𝐁𝐲 : {search}\n\n💬 𝐒𝐞𝐚𝐫𝐜𝐡𝐞𝐝 𝐁𝐲 : {message.from_user.mention}\n\n⏳ 𝐒𝐞𝐚𝐫𝐜𝐡𝐞𝐝 𝐓𝐢𝐦𝐞 : {remaining_seconds} sᴇᴄᴏɴᴅs\n\nᴘᴏᴡᴇʀᴇᴅ ʙʏ ☞ : {message.chat.title} \n\n⚠️ ᴀꜰᴛᴇʀ 5 ᴍɪɴᴜᴛᴇꜱ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ᴅᴇʟᴇᴛᴇᴅ 🗑️\n\n</b>"
            cap+="<b><u>⬇️ Your Movie Files ⬇️</u></b>\n\n"
            for file in files:
